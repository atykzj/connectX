...
def AB_agent_ScoreM(obs, config):
    ...
    def drop_piece(grid, col, mark, config):
        ...
        cell_state_rewards = score_mat[row][col]
        return next_grid, cell_state_rewards

    # Uses minimax to calculate value of dropping piece in selected column
    def score_move(grid, col, mark, config, nsteps):
        next_grid, cell_state_rewards = drop_piece(grid, col, mark, config)
        alpha, beta = -1e4, 1e6
    ...
    def minimax(node, depth, maximizingPlayer, mark, alpha, beta, config):
        ...
        if maximizingPlayer:
            value = -np.Inf
            # Add predefined score to value
            move = None
            for col in valid_moves:
                child, cell_state_rewards = drop_piece(node, col, mark, config)
                child_value, _ = minimax(child, depth-1, False, mark, alpha, beta, config)
                # # Add state rewards
                # child_value += cell_state_rewards
            ...
            move = None
            for col in valid_moves:
                child, cell_state_rewards = drop_piece(node, col, mark%2+1, config)
    ...
    # scores = dict(zip(valid_moves, [score_move(grid, valid_moves[col], obs.mark, config, N_STEPS) + \
