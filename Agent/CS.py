def CS(obs, conf):
    ...
    def evaluate_cell(cell):
        """ 1. Evaluate qualities of the cell in how valuable it is to each player's patterns """
    ...
    def get_patterns(cell):
        """ 2A1. This function gets the swarm and opponent's patterns of each axis of the cell. 
        It calls the get_pattern function for each axis and concatenates the resulting lists."""
    ...
    def get_pattern(x, x_fun, y, y_fun, cells_remained):
        """ 2A2. pattern of marks in a given direction. It uses recursion to explore cells in the
        direction until the required number of cells is reached or until the end of the board is reached. """
        pattern = []
        x = x_fun(x)
        y = y_fun(y)
    ...
    def calculate_points(cell):
        """ 2B. Calculates the amounts of swarm's and opponent's correct patterns and adds them to cell's points. 
        It calls the evaluate_pattern function for each axis and updates the points accordingly."""
    ...
    def evaluate_pattern(points, pattern, mark, inarow):
        """3. Evaluates if returns the amount of points if the pattern has the required amount of marks and zeros. 
        It iterates over the cells in the pattern and counts the number of marks and zeros. 
        If the required number of marks and zeros is present, it adds points to the current amount of points 
        and returns the updated amount of points. """
    ...
    def explore_cell_above(cell, i):
        """2C. add positive or negative points from cell above (if it exists) to points of current cell """
        if (cell["y"] - i) >= 0:
